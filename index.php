...
// Thiết lập giá trị mặc định nếu chưa có banner trong DB
$banner_img = $banner ? $banner['image'] : 'https://media.giphy.com/media/l0HlHFRbmaZtBRhXG/giphy.gif';
$banner_content = $banner ? $banner['content'] : '';

// 2. TRUY VẤN HÀNG MỚI VỀ
$sql_new = "SELECT * FROM san_pham WHERE trang_thai = 'DANG_BAN' ORDER BY tao_luc DESC LIMIT 4";
$result_new = $conn->query($sql_new);

// 3. TRUY VẤN SẢN PHẨM BÁN CHẠY
$sql_bestseller = "SELECT sp.*, COALESCE(SUM(ctdh.so_luong), 0) as tong_ban 
                   FROM san_pham sp 
                            ...
                            <a href="productdetail.php?id=<?php echo $row['id']; ?>" class="text-decoration-none text-dark">
                                <div class="card product-card border-0 shadow-sm h-100">
                                    <div class="product-image-container">
                                        ...
                                        <img src="<?php echo $row['anh_dai_dien_url']; ?>" alt="<?php echo $row['ten_san_pham']; ?>">
                                    </div>
